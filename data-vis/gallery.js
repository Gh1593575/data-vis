function Gallery() {
  ...
  this.addVisual = function (vis) {
    ...
    menuItem.mouseOver(function () {
      select('#' + vis.id).addClass('hover');
    });

    menuItem.mouseOut(function () {
      select('#' + vis.id).removeClass('hover');
    });
    ...
    menuItem.mouseClicked(function () {
      ...
      select('#' + vis.id).addClass('selected');
      self.selectVisual(vis.id);
    });
  ...
  this.selectVisual = function (visId) {
    ...
    if (visIndex != null) {
      ...
      // If data hasn't loaded yet, poll until it has then re-run setup().
      // This handles auto-selection at startup before CSV files arrive.
      if (this.selectedVisual.hasOwnProperty('loaded') &&
        !this.selectedVisual.loaded) {
        var vis = this.selectedVisual;
        var retry = setInterval(function () {
          if (vis.loaded) {
            clearInterval(retry);
            if (vis.hasOwnProperty('setup')) vis.setup();
          }
        }, 100);
      }
