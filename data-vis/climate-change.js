function ClimateChange() {
  ...
  // Update the CSS filled-track gradient so the purple portion
  // correctly reflects the current slider value.
  this.updateSliderFill = function(slider) {
    var elt = slider.elt;
    var min = parseFloat(elt.min);
    var max = parseFloat(elt.max);
    var val = parseFloat(elt.value);
    var pct = ((val - min) / (max - min)) * 100;
    elt.style.backgroundSize = pct + '% 100%';
  };
  /* End own code */
  ...
  this.draw = function() {
    ...
    this.positionSlider(this.startSlider, width - 300, 20);
    this.positionSlider(this.endSlider,   width - 150, 20);
    // Keep the purple filled-track in sync with each slider value.
    this.updateSliderFill(this.startSlider);
    this.updateSliderFill(this.endSlider);
    /* End own code */
