...
function OccupationRadar() {
  ...
  this.repositionSelects = function () {
    var selectYs = [20, 68, 116];
    for (var s = 0; s < this.selects.length; s++) {
      this.selects[s].elt.style.display = 'block';
      this.positionSelect(this.selects[s], width - 180, selectYs[s]);
    }
  };
  ...
  this.setup = function () {
    ...
    var self     = this;
    var selectYs = [20, 68, 116];  // vertical positions on canvas

    for (var s = 0; s < 3; s++) {
      var sel = createSelect();
      sel.elt.style.display = 'none';
      this.positionSelect(sel, width - 180, selectYs[s]);

      // Colour-code the dropdown with its team colour.
      var tc = this.teamColors[s];
      sel.elt.style.background = 'linear-gradient(135deg, rgba(' + tc.r + ',' + tc.g + ',' + tc.b + ',0.85), rgba(' + tc.r + ',' + tc.g + ',' + tc.b + ',0.55))';
      sel.elt.style.color  = '#ffffff';
      sel.elt.style.border = '1.5px solid rgba(' + tc.r + ',' + tc.g + ',' + tc.b + ',0.7)';
  ...
  this.draw = function () {
    ...
    // (Colour indicators replaced by coloured dropdown backgrounds.)
  ...
  this.drawGrid = function () {
    ...
    for (var i = 0; i < n; i++) {
      ...
      noStroke(); fill(210); textSize(11);

      // Align label based on which side of the chart it falls on.
      var lx = this.center.x + (this.radius + 34) * cos(a);
      var ly = this.center.y + (this.radius + 34) * sin(a);
      var cosA = cos(a);
      if (cosA > 0.3)       textAlign(LEFT,   CENTER);
      else if (cosA < -0.3) textAlign(RIGHT,  CENTER);
