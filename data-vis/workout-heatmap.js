...
function WorkoutHeatmap() {
  ...
  this.layout = {
    gridLeft: 100,   // recomputed in setup() — left-aligned
    gridTop: 82,    // title area + horizontal-header row
    cellW: 68,    // recomputed in setup() based on canvas width
    cellH: 32,    // recomputed in setup() based on canvas height
  };

  // Padding constants — give the grid visible breathing room on all sides.
  this.PAD_LABEL = 85;   // pixels reserved left of cells for genre labels
  this.PAD_H = 5;    // horizontal padding left & right of the full block
  ...
  this.draw = function () {
    ...
    // Column headers (countries) — horizontal, centred above each column.
    for (var c = 0; c < this.countries.length; c++) {
      var cx = this.layout.gridLeft + c * this.layout.cellW + this.layout.cellW / 2;
      var cy = this.layout.gridTop - 12;
      noStroke();
      fill(255);
      textAlign(CENTER, BOTTOM);
      textSize(12);
      text(this.countries[c], cx, cy);
    }
  ...
  this.drawLegend = function (eased) {
    ...
    var legendW = this.countries.length * this.layout.cellW - 4;  // match full grid width
    var legendH = 12;
